...
var Snapdeal=Snapdeal||{};Snapdeal.ServiceWorkerPush=(function(E,J){var G,K,A,C,D,L,H,B,F,I;F=E("#chromePushAPIkey");F=(F&&F.val())?F.val():"BCbiu7PrK6jg4yH1SOoz8KLljHg6RuSxrC4JIubRNddVpr4M6KHnbuLxNoAxbgaM2aAXaLuFAZKpApqKOY-Bq1E";I=["HomePage","ProductDetailPage","categoryPage","searchCategory","searchResult","offersPage","csfDarwin","brandPage","brandCategory","marketingLandingPage"];C={isPushEnabled:(Snapdeal.Cookie.get("sdCPW")=="true"?true:false),isPushEnabledSwitch:((E("#isPushEnabled").val()=="true"&&window.pageName&&I.indexOf(window.pageName)!=-1)?true:false),pushTimer:E("#pushTimer").val()};A={pushContainer:E(".push-overlay"),pushAllow:E("#pushAllow"),pushDenied:E("#pushDenied"),pushClose:E(".push-close")};K={_init:function(){K._registerServiceWorker()},_bindEvents:function(){A.pushAllow.on("click",function(){K._subscribe(false,false);A.pushContainer.addClass("hidden");return false});A.pushDenied.on("click",function(){Snapdeal.Cookie.set("sdCPW","false",7);A.pushContainer.addClass("hidden");return false});E(".push-container").mouseover(function(){clearTimeout(L)}).mouseout(function(){L=window.setTimeout(function(){A.pushContainer.addClass("hidden")},parseInt(C.pushTimer))});navigator.serviceWorker.onmessage=function(M){console.log("Data Received : ",M.data);var N=JSON.parse(M.data);N.value=JSON.parse(N.value);if(N){switch(N.type){case"trigger":SDLogger.log("render","notificationTriggered",null);break;case"render":SDLogger.log("render","notificationReceived",N.value);break;case"click":SDLogger.log("clickStream","notificationClick",N.value);break;default:console.log("didn't match any case");break}}}},_urlB64ToUint8Array:function(R){var P="=".repeat((4-R.length%4)%4);var M=(R+P).replace(/\-/g,"+").replace(/_/g,"/");var Q=window.atob(M);var O=new Uint8Array(Q.length);for(var N=0;N<Q.length;++N){O[N]=Q.charCodeAt(N)}return O},_registerServiceWorker:function(){if("serviceWorker" in navigator&&"PushManager" in window){navigator.serviceWorker.register("/sw.js").then(function(N){var M=window.Notification?window.Notification.permission:"denied";K._errorLog("notifPermission",M);H=N;if(C.isPushEnabledSwitch){D=setInterval(function(){navigator.serviceWorker.ready.then(function(O){clearInterval(D);H=O;K._initialiseState()})["catch"](function(O){K._errorLog("serviceWorkerReady",O.message)})},300)}else{A.pushContainer.addClass("hidden")}})["catch"](function(M){K._errorLog("serviceWorkerRegister",M.message)})}},_initialiseState:function(){H.pushManager.getSubscription().then(function(N){B=!(N===null);K._bindEvents();var M=window.Notification?window.Notification.permission:"denied";if(B){if(!!Snapdeal.Cookie.get("sdCPW")){K._sendSubscriptionToServer(N,false,false)}else{N.unsubscribe().then(function(O){if(O){K._subscribe(true,false)}else{K._errorLog("unSubscriptionError","empty response")}})["catch"](function(O){K._errorLog("unSubscriptionError",O.message)})}}else{if(M=="granted"){K._errorLog("nullSubscription",Snapdeal.Cookie.get("sdCPW"))}if(M==="denied"){return }else{if(C.isPushEnabled||M=="granted"){K._subscribe(false,true)}else{if(M=="default"&&Snapdeal.Cookie.get("sdCPW")!="false"&&window.pageName=="HomePage"){A.pushContainer.removeClass("hidden");L=window.setTimeout(function(){A.pushContainer.addClass("hidden")},6000)}}}}})["catch"](function(M){K._errorLog("getSubscriptionError",M.message)})},_sendSubscriptionToServer:function(U,R,O){U=JSON.parse(JSON.stringify(U));var Q=U.endpoint.split("/");var T=Q[Q.length-1];var N=(navigator.connection||navigator.mozConnection||navigator.webkitConnection)||"";var P,M=0;try{P={notificationId:T||"",networkType:N,latitude:"",longitude:"",notificationAllowed:true,p256dh:U.keys.p256dh||"",auth:U.keys.auth||"",endPointUrl:U.endpoint};if(!window.sdLoggerInitialised){M=150}setTimeout(function(){if(window.sdLoggerSwitch){if(!window.sdLoggerInitialised&&typeof SDLogger==="object"&&typeof SDLogger.init==="function"){SDLogger.init(SDLoggerConfig)}if(R){SDLogger.log("render","reSubscribeNotification",{notificationId:P.notificationId})}if(O){SDLogger.log("render","autoSubscribeNotification",{notificationId:P.notificationId})}SDLogger.log("webEvent","browserHeartBeat",P)}},M)}catch(S){K._errorLog("SendingHeartbeatData",S.message)}},_errorLog:function(N,O){var M=0;if(!window.sdLoggerInitialised){M=200}setTimeout(function(){if(!window.sdLoggerInitialised&&typeof window.SDLogger==="object"&&typeof SDLogger.init==="function"&&window.SDLoggerConfig){SDLogger.init(SDLoggerConfig)}if(window.sdLoggerInitialised){SDLogger.log("clickStream","eventLoggingLogging",{type:"notificationSWErrorWeb",name:N,values:O})}},M)},_subscribe:function(M,N){var O=K._urlB64ToUint8Array(F);H.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:O}).then(function(P){Snapdeal.Cookie.set("sdCPW","true",365);Snapdeal.Cookie.set("pushD","",-1);Snapdeal.Cookie.set("sdChromePush","",-1);K._sendSubscriptionToServer(P,M,N);B=true})["catch"](function(P){K._errorLog("pushManagerSubscribe",P.message)})}};G={init:function(){return K._init.apply(this,arguments)}};return G})(window.jQuery,Snapdeal);Snapdeal.ServiceWorkerPush.init();
